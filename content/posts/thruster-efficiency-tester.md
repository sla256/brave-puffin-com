...
## Intro
While working on various versions of Puffin in the last 5 years, I ran several dozens of different underwater thruster efficiency tests - to find the best combination of the motor, propeller, ESC, mounting, etc.
...
* 20 kg load cell mounted on a rigid aluminum bracket and 1/4" steel angle; calibrated in place.
...
So we mounted a particular motor + propeller combo we want to test, and ran the thing. It automatically incremented the power to the motor over the course of several minutes, while measuring and recording the results on the SD card. Now we want to analyze the data - what are we looking at here?
...
For that we need to look at the **efficiency, the blue chart** - how much thrust are we getting for a unit of power? As you can see, there is a very distinct peak efficiency for this motor + prop combo. Right around 8-9W it reaches ~80 gram/watt factor.

The thrust at this point is not very big, only 0.7kg - but it's the most efficient regime - a cruise mode.  It could be good for long nights or cloudy days when the boat needs to conserve power yet keep moving.
...
Now, let's look at various factors that could affect the efficiency.
...
![9" APC prop during high power phase of the test](/img/9inch-prop-turbulence-clip-15fps.gif)
 ...
 The efficiency gap might have disappeared because of that turbulence, which may not be as large of a factor in open water conditions. To be continued...

### ESC 1 (30A) vs. ESC 2 (40A)

Testing a couple of generic ESC's, both working OK with a high pole construction of the underwater motor.
...
* It's overall design was much less "swept" that 9" one's
